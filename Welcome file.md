...
1. User Agent apps present users with claims they can request from Issuers, which are represented by Claim Manifests that are found by crawling Identity Hubs or explicit ingestion into a UA's directory (as noted above). Alice, a DID user, chooses to initiate the claim request with MasterCard. 

2. MasterCard's MDL ClaimManifest specifies a set of data and prerequisite claims Alice needs to submit in order for MasterCard to process her claim request. Alice's UA presents her with an interface that helps her input the right data and select relevant prerequisite claims.

3. When Alice has filled out all the fields to meet the requirements, her request is sent to the Identity Hub of the Issuer for processing, which the Enterprise Agent handles. The EA is a headless agent with code that handles business logic and activities specific to an Issuer, Assuming Alice filled out the fields correctly and sent acceptable prerequisite claims, MasterCard will return her an MDL claim.
...
participant Alice's User Agent as UA
participant MasterCard Enterprise Agent as EA
participant MasterCard Hub as MH

Alice's User Agent-->Alice's User Agent: 1. Alice chooses the MDL claim
Alice's User Agent-->Alice's User Agent: 2. Alice assembles required data
Alice's User Agent-->MH: 3. UA sends an IssueClaimAction
MH-->EA: 4. EA acts on the request 
EA-->MH: 5. EA stores the claim in its Hub
EA-->AH: 6. EA returns the claim to Alice
```
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNjU2NTQxMzQ0XX0=
-->